...
@section('title', 'Kegiatan Sosial TBC')
...
@section('content')
    ...
    <section class="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
        <div class="max-w-6xl mx-auto px-4 py-3">
            <form method="GET" action="{{ route('kegiatan-sosial.index') }}"
                ...
                @if(request()->hasAny(['search', 'status']))
                    <a href="{{ route('kegiatan-sosial.index') }}" class="text-sm text-gray-500 hover:text-gray-700">Reset</a>
                @endif
            </form>
        </div>
    </section>
                                ...
                                <article
                                    ...
                                    <div class="aspect-video overflow-hidden bg-gray-100 relative">
                                        ...
                                        <div class="absolute top-3 left-3">
                                            ...
                                            @endphp
                         <span
                                                class="{{ $colorMap[$item->status] ?? 'bg-gray-500' }} text-white text-xs font-medium px-2 py-0.5 rounded-full">
                                                {{ $item->status_label }}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-5">
                                        ...
                                        {{-- Statistik ringkasan (jika sudah selesai) --}}
                                        @if($item->ringkasan)
                                            <div class="flex gap-3 text-xs text-gray-500 mb-4 bg-gray-50 rounded-lg px-3 py-2">
                                                <span>👥 {{ $item->ringkasan->jumlah_peserta }} peserta</span>
                                                <span>·</span>
                                                <span>🧑‍⚕️ {{ $item->ringkasan->jumlah_kader }} kader</span>
                                            </div>
                                        @endif

                                        {{-- Tombol --}}
                                        <a href="{{ route('kegiatan-sosial.show', $item->slug) }}"
